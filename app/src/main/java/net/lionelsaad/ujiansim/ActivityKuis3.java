...
public class ActivityKuis3 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setSubtitle("Ujian Ketiga");
        btnLanjut = (Button)findViewById(R.id.btn_lanjut_soal_ketiga);
        btnLanjut.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(ActivityKuis3.this, ActivityKuis4.class);
                startActivity(intent);
            }
        });
        mLinearLayout = (LinearLayout)findViewById(R.id.ll_ketiga);
        mContext = getApplicationContext();
        mActivity = ActivityKuis3.this;
        radiogroup = (RadioGroup) findViewById(R.id.rbg_ketiga);
    }

    public void pilihanKetiga (View view){
        Boolean checked = ((RadioButton) view).isChecked();
        switch (view.getId()) {
            case R.id.soal_ketiga_a:
                if (checked)
                    jawabanSalah(view);
                break;

            case R.id.soal_ketiga_b:
                if (checked)
                    jawabanBenar(view);
                break;
            case R.id.soal_ketiga_c:
                if (checked)
                    jawabanSalah(view);
                break;
        }
    ...
    private void showDialog(){
        ...
        alertDialogBuilder
                .setMessage("Apakah anda yakin ingin kembali ke soal kedua?")
                .setIcon(R.mipmap.ic_apps)
                .setCancelable(false)
                .setPositiveButton("Ya",new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog,int id) {
                        ActivityKuis3.this.finish();
                    }
                })
