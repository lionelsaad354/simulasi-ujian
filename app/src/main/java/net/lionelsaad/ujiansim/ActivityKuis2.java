...
public class ActivityKuis2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setSubtitle("Ujian Kedua");
        btnLanjut = (Button)findViewById(R.id.btn_lanjut_soal_kedua);
        btnLanjut.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(ActivityKuis2.this, ActivityKuis3.class);
                startActivity(intent);
            }
        });

        mLinearLayout = (LinearLayout)findViewById(R.id.ll_kedua);
        mContext = getApplicationContext();
        mActivity = ActivityKuis2.this;
        radiogroup = (RadioGroup) findViewById(R.id.rbg_kedua);
    }
    public void pilihanKedua (View view){
        Boolean checked = ((RadioButton) view).isChecked();
        switch (view.getId()) {
            case R.id.soal_kedua_a:
                if (checked)
                    jawabanBenar(view);
                break;

            case R.id.soal_kedua_b:
                if (checked)
                    jawabanSalah(view);
                break;
            case R.id.soal_kedua_c:
                if (checked)
                    jawabanSalah(view);
                break;
        }
    ...
    private void showDialog(){
        ...
        alertDialogBuilder
                .setMessage("Apakah anda yakin ingin kembali ke soal pertama?")
                .setIcon(R.mipmap.ic_apps)
                .setCancelable(false)
                .setPositiveButton("Ya",new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog,int id) {
                        ActivityKuis2.this.finish();
                    }
                })
